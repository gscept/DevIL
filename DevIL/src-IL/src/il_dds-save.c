...
ILuint GetCubemapInfo(ILimage* image, ILint* faces)
{
	...
	iGetIntegervImage(image, IL_NUM_FACES, (ILint*) &srcImagesCount);
...
ILboolean iSaveDdsInternal()
{
	...
	if (!WriteHeader(iCurImage, DXTCFormat, CubeFlags))
		return IL_FALSE;

	// if we have an uncompressed format, or a format which require the DX10 header, then always write an extended header
	if (DXTCFormat == IL_DXGI_UNCOMPRESSED || DXTCFormat >= IL_DXT1A_sRGB || DXTCFormat <= IL_BPTC_sRGB)
	{
		if (!WriteExtendedHeader(iCurImage, DXTCFormat, CubeFlags))
			return IL_FALSE;
	}
	...
	for (i = 0; i <= numFaces; ++i) {
		for (counter = 0; counter <= numMipMaps; counter++) {
			...
			// only attempt compression if we have a compressed format as output
			if (DXTCFormat != IL_DXGI_UNCOMPRESSED)
			{
			...
			{
				// simply put image data into file
...
ILboolean WriteHeader(ILimage *Image, ILenum DXTCFormat, ILuint CubeFlags)
{
	...
	Depth = Image->Depth;
	Faces = ilGetInteger(IL_NUM_FACES);

	// we can't have a cube 3D texture
	if (Depth > 1 && Faces > 1) {
	...
	switch (DXTCFormat)
	{
		...
		case IL_RXGB:
			...
			break;
		case IL_DXGI_UNCOMPRESSED:
		case IL_DXT1A_sRGB:
		case IL_DXT3_sRGB:
		case IL_DXT5_sRGB:
		case IL_BPTC:
		case IL_BPTC_sRGB:
			FourCC = IL_MAKEFOURCC('D','X','1','0');
			break;
		default:
			// Error!
	...
	*/
	...
	if (CubeFlags != 0) {
		...
		ddsCaps2 |= 0x200 | 0x400 | 0x800 | 0x1000 | 0x2000 | 0x4000 | 0x8000;
	}
...
// Internal function used to save a DX10 header, can't actually handle ALL DX10 types, but can translate from IL type and format into a viable DX10 format
// All types must be 32-bit aligned
ILboolean WriteExtendedHeader(ILimage *Image, ILenum DXTCFormat, ILuint CubeFlags)
{
	ILuint DXGIFormat, resourceDimension, cube, arraySize, miscFlags2;
	ILimage* Temp;
	...
	case IL_DXT1A_sRGB:
		...
		break;
	case IL_DXT3_sRGB:
		DXGIFormat = 75;					// DXGI_FORMAT_BC2_UNORM_SRGB              
		break;
	case IL_DXT5_sRGB:
		DXGIFormat = 78;					// DXGI_FORMAT_BC3_UNORM_SRGB              
		break;
	case IL_BPTC:
	...
	case IL_DXGI_UNCOMPRESSED:
	{
		...
		case IL_RGBA:
			...
			case IL_UNSIGNED_BYTE:
				DXGIFormat = 30;			// DXGI_FORMAT_R8G8B8A8_UINT
				break;
			case IL_UNSIGNED_SHORT:
				DXGIFormat = 12;			// DXGI_FORMAT_R16G16B16A16_UINT
				break;						           
			case IL_UNSIGNED_INT:
				DXGIFormat = 3;				// DXGI_FORMAT_R32G32B32A32_UINT     
				break;
			case IL_BYTE:
				DXGIFormat = 32;			// DXGI_FORMAT_R8G8B8A8_SINT               
				break;
			case IL_SHORT:
				DXGIFormat = 14;			// DXGI_FORMAT_R16G16B16A16_SINT                          
				break;
			case IL_INT:
				DXGIFormat = 4;				// DXGI_FORMAT_R32G32B32A32_SINT           
				break;
			case IL_HALF:
				DXGIFormat = 10;			// DXGI_FORMAT_R16G16B16A16_FLOAT          
				break;
			case IL_FLOAT:
				DXGIFormat = 2;				// DXGI_FORMAT_R32G32B32A32_FLOAT
				break;
			}
			break;
		case IL_RGB:
			...
			{
			case IL_UNSIGNED_BYTE:
				DXGIFormat = 0;				// DXGI_FORMAT_UNKNOWN                     
				break;
			case IL_UNSIGNED_SHORT:
				DXGIFormat = 0;				// DXGI_FORMAT_UNKNOWN                     
				break;
			case IL_UNSIGNED_INT:
				DXGIFormat = 7;				// DXGI_FORMAT_R32G32B32_UINT                   
				break;
			case IL_BYTE:
				DXGIFormat = 0;				// DXGI_FORMAT_UNKNOWN               
				break;
			case IL_SHORT:
				...
				break;
			case IL_INT:
				DXGIFormat = 8;				// DXGI_FORMAT_R32G32B32_SINT           
				break;
			case IL_HALF:
				DXGIFormat = 0;				// DXGI_FORMAT_UNKNOWN          
				break;
			case IL_FLOAT:
				DXGIFormat = 6;				// DXGI_FORMAT_R32G32B32_FLOAT             
				break;
			}
			break;
		case IL_BGRA:
		...
		case IL_BGR:
			...
			{
			case IL_UNSIGNED_BYTE:
				DXGIFormat = 88;			// DXGI_FORMAT_B8G8R8X8_UNORM                                                 
				break;
			default:
				DXGIFormat = 0;				// DXGI_FORMAT_UNKNOWN
				break;
		...
		case IL_RG:
			...
			{
			case IL_UNSIGNED_BYTE:
				DXGIFormat = 50;			// DXGI_FORMAT_R8G8_UINT                                        
				break;
			case IL_UNSIGNED_SHORT:
				DXGIFormat = 36;			// DXGI_FORMAT_R16G16_UINT                                                          
				break;
			case IL_UNSIGNED_INT:
				DXGIFormat = 17;			// DXGI_FORMAT_R32G32_UINT                                                        
				break;
			case IL_BYTE:
				DXGIFormat = 52;			// DXGI_FORMAT_R8G8_SINT                                  
				break;
			case IL_SHORT:
				DXGIFormat = 38;			// DXGI_FORMAT_R16G16_SINT                                                              
				break;
			case IL_INT:
				DXGIFormat = 18;			// DXGI_FORMAT_R32G32_SINT           
				break;
			case IL_HALF:
				DXGIFormat = 34;			// DXGI_FORMAT_R16G16_FLOAT                                             
				break;
			case IL_FLOAT:
				DXGIFormat = 16;			// DXGI_FORMAT_R32G32_FLOAT                                                
				break;
			}
			break;
		case IL_RED:
			...
			{
			case IL_UNSIGNED_BYTE:
				DXGIFormat = 62;			// DXGI_FORMAT_R8_UINT                                                             
				break;
			case IL_UNSIGNED_SHORT:
				DXGIFormat = 57;			// DXGI_FORMAT_R16_UINT                                         
				break;
			case IL_UNSIGNED_INT:
				DXGIFormat = 42;			// DXGI_FORMAT_R32_UINT                                       
				break;
			case IL_BYTE:
				DXGIFormat = 64;			// DXGI_FORMAT_R8_SINT                                                                            
				break;
			case IL_SHORT:
				DXGIFormat = 59;			// DXGI_FORMAT_R16_SINT                                             
				break;
			case IL_INT:
				DXGIFormat = 43;			// DXGI_FORMAT_R32_SINT           
				break;
			case IL_HALF:
				DXGIFormat = 54;			// DXGI_FORMAT_R16_FLOAT                             
				break;
			case IL_FLOAT:
				DXGIFormat = 41;			// DXGI_FORMAT_R32_FLOAT                                
				break;
			}
			break;
		}
	...
	if (Image->Width >= 1 && Image->Height == 1 && Image->Depth == 1)
		resourceDimension = 2;														// 1D texture
	else if (Image->Width >= 1 && Image->Height > 1 && Image->Depth == 1)
		resourceDimension = 3;														// 2D texture
	else if (Image->Width >= 1 && Image->Height >= 1 && Image->Depth > 1)
		resourceDimension = 4;														// 3D texture
	else
		resourceDimension = 1;														// buffer, maybe this is somewhat stupid

	cube = CubeFlags > 0 ? 0x4 : 0x0;

	arraySize = 0;
	Temp = Image;
	if (Temp = Temp->Next) do arraySize++;
	while (Temp = Temp->Next);

	Temp = Image;
	if (Temp = Temp->Faces) do arraySize++;
	while (Temp = Temp->Faces);
	miscFlags2 = 0x0;

	SaveLittleUInt(DXGIFormat);				// dxgiFormat
	SaveLittleUInt(resourceDimension);		// resourceDimension
	SaveLittleUInt(cube);					// miscFlag
	SaveLittleUInt(arraySize);				// arraySize
	SaveLittleUInt(miscFlags2);				// miscFlags2

	// if 0, then the format has failed
	return DXGIFormat;
}
